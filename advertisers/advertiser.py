...
class Advertiser(commands.Cog):
    def __init__(self, bot: Red):
        self.bot = bot
        self.data = Config.get_conf(
            self, identifier=3467369396940, force_registration=True
        )
    ...
    @commands.guild_only()
    @commands.group()
    async def advertiser(self, ctx):
        """Advertiser System"""
        if ctx.invoked_subcommand is None:
            pass

    @checks.admin()
    @advertiser.command(name="addpoint", aliases=["apoint"])
    async def _addpoint(self, ctx, user: discord.Member, point: int):
        """Add an advertiser point to the user."""
        ...
        await ctx.send(f"Added {point} advertiser points to {user.mention}.")

    @checks.admin()
    @advertiser.command(name="removepoint", aliases=["rpoint"])
    async def _removepoint(self, ctx, user: discord.Member, point: int):
        """Remove a advertiser point to the user."""
        data = await self.data.member(user).weekly_points()
        data2 = await self.data.member(user).points()
        ...
        await ctx.send(f"Removed {point} advertiser points to {user.mention}.")

    @checks.admin()
    @advertiser.command(name="reset")
    ...
    @checks.admin()
    @advertiser.command(name="channel")
    async def _channel(self, ctx, channel: discord.TextChannel = None):
        """Specify a channel for advertisers."""
        channel = None if not channel else channel
        if channel:
            await self.data.guild(ctx.guild).channel.set(channel.id)
            await ctx.send(f"The advertising channel has been set to {channel.mention}.")
        else:
            await self.data.guild(ctx.guild).channel.set(None)
            await ctx.send(f"Reset the advertising channel!")

    @advertiser.command(name="weekly")
    async def _weekly(self, ctx):
        """ View weekly leaderboard for advertisers."""
        data = await self.data.all_members(ctx.guild)
        if not data:
        ...
        for rank in sorted_data:
            ...
            if n > 10:
                break
        embed = discord.Embed(
            title="Weekly Advertiser Leaderboard",
            description=message,
    ...
    @advertiser.command(name="alltime", aliases=["all"])
    async def _all(self, ctx):
        """ View All time leaderboard for advertisers."""
        data = await self.data.all_members(ctx.guild)
        if not data:
            return await ctx.send("There is nothing to see here.")
        ...
        for rank in sorted_data:
            ...
            if n == 11:
                break
        embed = discord.Embed(
            title="Alltime Advertiser Leaderboard",
            description=message,
            color=discord.Colour(await ctx.bot._config.color()),
        )
        embed.set_author(name=ctx.guild, icon_url=ctx.guild.icon_url)
        embed.set_thumbnail(url=ctx.guild.icon_url)
        embed.set_footer(text="Top 10 alltime advertisers")
        await ctx.send(embed=embed)

    @advertiser.command(name="stats")
    async def _stats(self, ctx, member: discord.Member = None):
        ...
        if not data and not data2:
            em = discord.Embed(
                description="Either the advertiser channel isn't set either you or the user don't have any data."
            )
            return await ctx.send(embed=em)
    ...
    async def on_message(self, message):
        ...
        advertiser_channel = await self.data.guild(guild).channel()
        if not advertiser_channel:
            return

        if advertiser_channel != message.channel.id:
            return
